## XFRP lexer.
## The lexer will produce a sequence of tokens with code fragments.
##
## **See also:**
## * `tokens <tokens.html>`_ for XFRP tokens
## * `codeinfos <codeinfos.html>`_ for code fragments
## * `nimly <https://github.com/loloicci/nimly>`_ for lexer generator macro ``niml``
...
when defined(nimdoc):
  const xfrpLex: LexData[XfrpToken] = default(LexData[XfrpToken])
